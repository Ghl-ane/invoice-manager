...
<div class="bg-white rounded-xl border border-slate-200">
    ...
    <div class="text-center py-16">
        ...
        <a href="{{ route('invoices.create') }}" class="text-amber-500 text-sm hover:underline">Create your first invoice →</a>
    </div>
    @else
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-50">
            ...
            <tr class="hover:bg-slate-50 transition-colors">
                <td class="px-6 py-4">
                    <a href="{{ route('invoices.show', $invoice) }}" class="font-medium text-slate-700 hover:text-amber-500">
                        {{ $invoice->invoice_number }}
                    </a>
                </td>
                <td class="px-6 py-4 text-slate-500">{{ $invoice->client->name ?? '—' }}</td>
                <td class="px-6 py-4 text-slate-500">{{ \Carbon\Carbon::parse($invoice->issue_date)->format('M d, Y') }}</td>
                ...
                <td class="px-6 py-4 text-right font-semibold text-slate-700">${{ number_format($invoice->total, 2) }}</td>
                <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('invoices.show', $invoice) }}" class="text-xs text-slate-500 hover:text-amber-500 px-3 py-1.5 rounded-lg hover:bg-amber-50 transition-colors">View</a>
                        <a href="{{ route('invoices.edit', $invoice) }}" class="text-xs text-slate-500 hover:text-blue-500 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition-colors">Edit</a>
                        <form action="{{ route('invoices.destroy', $invoice) }}" method="POST" onsubmit="return confirm('Delete this invoice?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="text-xs text-slate-500 hover:text-red-500 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors">Delete</button>
                        </form>
                    </div>
                </td>
            </tr>
